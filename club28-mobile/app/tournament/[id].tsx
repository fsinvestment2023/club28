...
export default function RegistrationScreen() {
  ...
  const handleJoin = async () => {
    ...
    if (user.wallet_balance < selectedLevel.fee) return Alert.alert("Insufficient Funds", "Please add money to your wallet.");
  ...
  // PRIZE DATA (Updated Labels)
  const prizes = selectedLevel ? [
    { rank: '1ST PLACE', amount: selectedLevel.p1, icon: '🥇', color: '#fbbf24' },
    { rank: '2ND PLACE', amount: selectedLevel.p2, icon: '🥈', color: '#9ca3af' },
    { rank: '3RD PLACE', amount: selectedLevel.p3, icon: '🥉', color: '#b45309' },
    { rank: 'PER MATCH WIN', amount: selectedLevel.per_match, icon: '💰', color: '#10b981' } // FIXED LABEL
  ] : [];
    ...
    <View style={{flex: 1, backgroundColor: '#F3F4F6'}}>
      ...
      <View style={styles.blueHeader}>
        <SafeAreaView edges={['top', 'left', 'right']}>
          ...
          {/* ADDED FORMAT (Singles/Doubles) HERE */}
      ...
      <ScrollView contentContainerStyle={{padding: 20, paddingBottom: 120}}>
        ...
        <View style={styles.card}>
            ...
            {categories.map((cat, idx) => (
                <TouchableOpacity 
                    key={idx} 
                    style={[styles.levelBtn, selectedLevel?.name === cat.name && styles.activeLevel]}
                    onPress={() => setSelectedLevel(cat)}
                >
                    <View>
                        <Text style={[styles.levelTitle, selectedLevel?.name === cat.name && {color:'#2563eb'}]}>{cat.name}</Text>
                        <Text style={{fontSize:10, color:'#999'}}>Entry Fee</Text>
                    </View>
        ...
        <View style={styles.card}>
            <View style={{flexDirection:'row', alignItems:'center', marginBottom:15}}>
                <Feather name="calendar" size={18} color="#2563eb" style={{marginRight:8}} />
                ...
                <View style={styles.scheduleBox}>
                    {schedule.map((row, idx) => (
                        <View key={idx} style={styles.scheduleRow}>
                            <Text style={styles.schLabel}>{row.label}</Text>
                            <Text style={styles.schValue}>{row.value}</Text>
                        </View>
                    ))}
                </View>
            ) : (
                <Text style={{color:'#ccc', fontStyle:'italic', marginBottom:10}}>Schedule coming soon.</Text>
            )}

            {tournament.venue && (
                <View style={{flexDirection:'row', marginTop: 10, alignItems:'center'}}>
                    <Feather name="map-pin" size={14} color="#666" style={{marginRight: 5}}/>
                    <Text style={{color:'#444', fontWeight:'bold'}}>{tournament.venue}</Text>
                </View>
            )}
        </View>
        ...
        {selectedLevel && (
            <View style={styles.card}>
                <View style={{flexDirection:'row', alignItems:'center', marginBottom:15}}>
                    <FontAwesome5 name="trophy" size={16} color="#fbbf24" style={{marginRight:8}} />
                    <Text style={styles.sectionTitle}>PRIZE POOL ({selectedLevel.name})</Text>
                </View>
                {/* VERTICAL LIST FOR CLEANER LOOK */}
                <View style={{gap: 10}}>
                    {prizes.map((p, i) => (
                        p.amount > 0 && (
                            <View key={i} style={styles.prizeRow}>
                                <View style={{flexDirection:'row', alignItems:'center'}}>
                                    <Text style={{fontSize:18, marginRight: 10}}>{p.icon}</Text>
        ...
        {isDoubles && (
            <View style={styles.card}>
                <Text style={styles.sectionTitle}>DOUBLES PARTNER</Text>
                <TextInput 
                    style={styles.input} 
                    placeholder="Partner Team ID (e.g. SA25)" 
                    value={partnerId}
      ...
      <View style={styles.footer}>
        <View>
            <Text style={{fontSize:10, fontWeight:'bold', color:'#999'}}>TOTAL ENTRY FEE</Text>
            <Text style={{fontSize:24, fontWeight:'900', color:'#1f2937'}}>₹{selectedLevel?.fee || 0}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  // PRIZE STYLES (UPDATED TO ROW LAYOUT)

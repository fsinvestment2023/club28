...
import { Trophy, User, ChevronRight, Search, Bell, Home as HomeIcon, CheckCircle, LogOut, Activity, ChevronDown, ArrowLeft } from 'lucide-react';
...
// --- UPDATED: OngoingEvents (Mult-Event Support with URL Fix) ---
const OngoingEvents = ({ category, level, myTeamID }) => {
    ...
    const fetchData = async () => { 
        try { 
            ...
            const [schRes, scoreRes, rankRes] = await Promise.all([ 
                ...
                fetch(`http://127.0.0.1:8000/standings/${category}?level=${encodedLevel}`) 
            ]); 
    ...
    useEffect(() => { fetchData(); const interval = setInterval(fetchData, 10000); return () => clearInterval(interval); }, [category, level]);
...
const HomePage = ({ user, onRefresh }) => {
  ...
  // Set default tournament view based on user registration
  const [viewingTournament, setViewingTournament] = useState("Club 28 League");
  
  useEffect(() => { 
      if (user?.registrations?.length > 0) {
          setViewingTournament(user.registrations[0].tournament);
      }
  }, [user]);

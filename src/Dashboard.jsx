...
const Dashboard = () => {
  ...
  // --- STATE FOR CREATING/EDITING TOURNAMENT ---
  ...
  const fetchTournaments = async () => {
    try { const res = await fetch(`${API_URL}/tournaments`); setTournaments(await res.json()); } catch (e) {}
  };
  ...
  const fetchPlayers = async () => {
      try { const res = await fetch(`${API_URL}/admin/players`); setPlayers(await res.json()); } catch(e){}
  };
  ...
  const handleAddMoney = async () => {
      if (!walletTeamId || !walletAmount) return alert("Fill fields");
      const res = await fetch(`${API_URL}/admin/add-wallet`, {
  ...
  const openCreateModal = () => {
      setEditingId(null); setEventName(""); setEventType("League"); setEventStatus("Open");
      setCategories([{ name: "", fee: 0, p1: 0, p2: 0, p3: 0 }]); setIsModalOpen(true);
  };
  const openEditModal = (t) => {
      setEditingId(t.id); setEventName(t.name); setEventType(t.type); setEventStatus(t.status);
  ...
  const handleModalSubmit = async () => {
      ...
      const body = { id: editingId, name: eventName, type: eventType, status: eventStatus, settings: categories };
  ...
  const handleCreateMatch = async () => {
    ...
    const t1 = document.getElementById('new-t1').value || "Team 1";
    const t2 = document.getElementById('new-t2').value || "Team 2";
    const date = document.getElementById('new-date').value; 
    const time = document.getElementById('new-time').value;
    const group = document.getElementById('new-grp').value || "A";
    await fetch(`${API_URL}/admin/create-match`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ category: selectedTournament.name, group_id: group, t1, t2, date, time }) });
    fetchMatches();
  };
  const handleMatchUpdate = async (id) => {
      const t1 = document.getElementById(`t1-${id}`).value;
      ...
      await fetch(`${API_URL}/admin/edit-match-full`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ id, t1, t2, date, time, score }) });
      alert("Match Updated"); fetchMatches();
  };
  const handleDeleteMatch = async (id) => {
      if(!window.confirm("Delete this match?")) return;
      await fetch(`${API_URL}/admin/delete-match`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ id }) });
      fetchMatches();
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {tournaments.map(t => (
                    <div key={t.id} onClick={() => {setSelectedTournament(t); setActiveTab("MANAGE");}} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-all cursor-pointer relative group">
                        <button onClick={(e) => { e.stopPropagation(); openEditModal(t); }} className="absolute top-4 right-4 p-2 bg-gray-100 rounded-full text-gray-500 hover:bg-blue-100 hover:text-blue-600 z-10"><Edit2 size={16}/></button>
                        ...
                        <div className="flex gap-4 text-sm text-gray-500 mt-4 border-t border-gray-50 pt-4"><div><span className="block text-[10px] font-bold uppercase text-gray-300">Fee Starts</span>₹{t.fee}</div><div><span className="block text-[10px] font-bold uppercase text-gray-300">Max Prize</span>₹{t.prize}</div></div>
                    </div>
                ))}
            </div>
        )}
            ...
            <div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50 text-gray-400 border-b border-gray-100 text-xs uppercase font-bold"><tr><th className="p-4">Name</th><th className="p-4">Phone</th><th className="p-4">Team ID</th><th className="p-4">Wallet Balance</th></tr></thead>
                        <tbody className="divide-y divide-gray-50">
                            {players.map(p => (
                                <tr key={p.id}>
                                    <td className="p-4 font-bold text-gray-800">{p.name}</td>
                                    <td className="p-4 text-gray-500 font-bold">{p.phone}</td>
        ...
        {activeTab === "MANAGE" && selectedTournament && (
            <div className="max-w-5xl">
                <div className="flex justify-between items-center mb-8">
                    <div><button onClick={() => setActiveTab("TOURNAMENTS")} className="text-xs font-bold text-gray-400 hover:text-gray-600 mb-1">← Back to Events</button><h2 className="text-3xl font-black text-blue-900">{selectedTournament.name}</h2></div>
                    <button onClick={fetchMatches} className="p-2 bg-white border rounded hover:bg-gray-50"><RefreshCw size={20}/></button>
                </div>
                {/* (Keep Match Add/Table from previous code, it's the same) */}
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8">
                    <h3 className="font-bold text-gray-700 mb-4 flex items-center gap-2"><Calendar size={18}/> Add Match</h3>
                    <div className="grid grid-cols-5 gap-3">
                        <input id="new-grp" placeholder="Group" className="p-2 bg-gray-50 rounded border text-sm font-bold"/>
                    ...
                    <button onClick={handleCreateMatch} className="w-full mt-3 bg-black text-white font-bold py-3 rounded-lg text-sm hover:bg-gray-800">+ Add Match</button>
                </div>
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <table className="w-full text-left text-sm">
